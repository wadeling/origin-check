...
func TestScoreSelfReportedModelOpusVsSonnet(t *testing.T) {
	score := scoreSelfReportedModel("claude-opus-4-8", "claude-sonnet-4-5")
	if score > 20 {
...
func TestScoreSelfReportedModelExactMatch(t *testing.T) {
	score := scoreSelfReportedModel("claude-opus-4-8", "claude-opus-4-8")
	if score < 95 {
...
func TestModelsConflictOpusSonnet(t *testing.T) {
	if !modelsConflict("claude-opus-4-8", "claude-sonnet-4-5") {
...
func TestReasoningStubWrongAnswer(t *testing.T) {
	...
	score := scoreTraits("$13", traits, "claude-opus-4-8")
	if score >= 80 {
...
func TestScoreTraitsSelfIDUsesStrictModelMatch(t *testing.T) {
	...
	score := scoreTraits("claude-sonnet-4-5", traits, "claude-opus-4-8")
	if score > 20 {
